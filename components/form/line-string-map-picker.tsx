...
const DRAFT_SOURCE_ID = "line-picker";
const DRAFT_LINE_LAYER_ID = "line-picker-line";
const DRAFT_POINT_LAYER_ID = "line-picker-points";

interface LineStringMapPickerProps {
  open: boolean;
  initialValue?: unknown;
  onClose: () => void;
  onConfirm: (value: LineGeometryValue) => void;
}
...
function resolvePickerBounds(mapView: MapViewConfig) {
  if (mapView.bounds) {
...
function removeDraftLayers(map: maplibregl.Map) {
  for (const layerId of [DRAFT_POINT_LAYER_ID, DRAFT_LINE_LAYER_ID]) {
    if (map.getLayer(layerId)) map.removeLayer(layerId);
  }
  if (map.getSource(DRAFT_SOURCE_ID)) map.removeSource(DRAFT_SOURCE_ID);
}

export function LineStringMapPicker({
  open,
  initialValue,
  onClose,
  onConfirm,
}: LineStringMapPickerProps) {
  ...
  const [draft, setDraft] = useState<LatLngValue[]>(() =>
    lineToLatLngPoints(initialValue),
  );
  ...
  useEffect(() => {
    if (!open) return;
    setDraft(lineToLatLngPoints(initialValue));
    setMetaError(null);
    setMapMeta(null);
    setMapLoading(true);
    ...
    Promise.all([resolveMapView(), getAdministrativeBoundary()])
      .then(([mapView, boundary]) => {
        if (!cancelled) setMapMeta({ mapView, boundary });
      })
      .catch((error: unknown) => {
        if (cancelled) return;
        setMetaError(
  ...
  useEffect(() => {
    ...
    const initial = lineToLatLngPoints(initialValue);
    ...
    map.on("load", () => {
      ...
      if (positions.length > 0) {
        ...
        map.fitBounds(resolvePickerBounds(mapMeta.mapView), {
          padding: PICKER_FIT_PADDING,
          maxZoom: PICKER_MAX_ZOOM,
          duration: 0,
        });
  ...
  const built = buildLineGeometryValue(draft);

  return (
    <Modal
      title="Vẽ đường trên bản đồ"
      size="xl"
      padding={false}
      onClose={onClose}
    >
      <div className="grid max-h-[90vh] grid-rows-[auto_minmax(0,1fr)_auto] overflow-hidden">
        ...
        <div className="space-y-4 overflow-y-auto px-5 py-4">
          <p className="text-sm text-muted">
            Click để thêm điểm tuyến theo thứ tự. Các điểm sẽ được nối thành
            đường; cần tối thiểu 2 điểm. Bấm Hoàn tất để lưu đường.
          </p>
          ...
          <div className="relative overflow-hidden rounded-xl border border-border">
            ...
            <div ref={containerRef} className="h-[min(38vh,360px)] w-full" />
          </div>
